...
def free(state,rules,cell,player) :
    ...
    if len(inter) == 0 :
        ...
        if intersection :
            cells=rules.get_rules_possibles_moves(inter[0], board)
            ...
            if inter :
                ...
                return 1
            print("\n\n il est dans un mill et peut pas au",cell)
            return False
        else :
            print("\n\n il n'est pas dans un mill mais il peut venir au",cell)
            return 1
...
def get_effective_cell_moves_improve(state,cell,player) :
    ...
    if player == -1:                 
        move_no=state.player1_forbidden_cell[1]
    else: 
        move_no=state.player2_forbidden_cell[1]

    if board.is_cell_on_board(cell):
        possibles_moves = MorabarabaRules.get_rules_possibles_moves(cell, board)
        effective_moves = []
        for move in possibles_moves:
            if board.is_empty_cell(move) and move != move_no:
                effective_moves.append(move)
        return effective_moves 
...
def Move(state,rules,actions,player_current,player) :
   ...
   if player == player_current :
        ...
        print("\n\n mes actions pour mills",actions_mill)
        if len(actions_mill) == 0 :
            return False
        elif len(actions_mill) == 1 :
            print("\n\n Charlot j'ai une seule action")
            if len(get_actions_can_make_mill(-1*player,state,rules)) == 0 and free(state,rules,actions_mill[0].get_action_as_dict()["action"]["to"],-1*player) == True :
                rest_moves=[x for x in which_move(state,rules,actions,player) if x not in save_mills(state,player)[1]]
                if len(rest_moves) != 0 :
                    at,to=ordinateur(state,rules,rest_moves,player,save_mills(state,player)[1])
                    print("\n\n at to",at,to)
                    if at != False :
                        print("je t'ai eu dans move")
                        for action in actions :
...
def move_and_occasion(actions,state,rules,player) :
    ...
    for action in actions :
        ...
        print("\n\n Je suis appellée Move and occasion",luck,pity)
    return luck,pity
...
def get_actions_can_make_mill(player,state,rules) :
    ...
    print("\n\n longueur actions",len(actions_mill))
    return actions_mill
...
def Make_occasion(rules,state,player,actions) :
    ...
    if actions[0].get_action_as_dict()["action_type"] == MorabarabaActionType.ADD :
        ...
        if len(get_actions_can_make_mill(player*-1,state,rules)) == 0 :
           ...
           print("\n\nle gar veut pas manger")

           pieces=which_move(state,rules,actions,player)
           print("\n\n voilà ce que je peux bouger",pieces)
           ...
           if at != False :
                ...
                at,to = block_mills(state,-player) 
                if at != False :
                    for action in actions :
                        if action.get_action_as_dict()['action']['to'] == to and action.get_action_as_dict()['action']['at'] == at :
                            return action
           ...
           if choice  :
                dangers = filter(state,choice,player)
                if dangers:
                    print('\n\nNE PAS BOUGER CES PIECES',dangers)
                    # supprimer les pieces qui empêchent l'adversaire de manger dans deux coup des pieces bougeables
                    for danger in dangers:
                        if danger in choice and len(choice) > 1:
                            choice.remove(danger)
                for action in actions :
