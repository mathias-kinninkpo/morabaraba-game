...
def try_again(state,player) :
    print("\n\nJE SUIS RENTRE DANS LE BLOCKAGE\n\n")
    board=state.get_board()
    pieces_mill = get_single_in_mill(state,-player,3)
    for piece in pieces_mill :
        cells=get_effective_cell_moves(board,piece)
        if len(cells) != 0 and len(set(MorabarabaRules.get_rules_possibles_moves(piece,board)) & set(board.get_player_pieces_on_board(Color(player)))) == 0 :
            for cell in cells :
                if board.is_empty_cell(cell) :
                    print("\n_n Je bloque mill",cell)
                    return cell
...
def ADD_and_Ocassion(state,player) :
    ...
    if (len(my_occasions) == 0 and len(adverse_occasions) == 0 ) :
        ...
        if state.get_player_info(player)["in_hand"] == 12 :
            ...
            double_occ = double(state,player)
            choice=[cell for cell in set(adverse_potential_occasion[1]) & set(double_occ)]
            if len(choice) == 0:
                print("\n\nJE NE PEUX PLUS BLOQUER EN ME CREANT D'OCCASION DANS LE FUTURE\n\n")
                choice = [adverse_potential_occasion[1]]
            else:
                print("\n\nJE PEUX BLOQUER EN ME CREANT D'OCCASION DANS LE FUTURE\n\n")
    elif len(my_occasions) != 0 and len(adverse_occasions) == 0 :
...
def two_double(state,player) : 
    ...
    for cell in cells:
        ...
        two = [cell for cell in cells if cells.count(cell) > 1]
        for mill in  state.get_board().mills():
            if len(set(mill) & set(two)) > 1:
                for piece in set(mill) & set(two):
                    if piece in two:
                        two.remove(piece)
        if two:
        ...
        if len(get_actions_can_make_mill(player*-1,state,rules)) == 0 :
           ...
           if at != False :
                ...
                if to != False :
                    ...
                    for action in actions :
                        if action.get_action_as_dict()['action']['at'] in ats  and action.get_action_as_dict()['action']['to'] == to:
                            return action
...
def get_possible_destination(player, state, pieces = []):
    player_mills = get_piece_mills(state,-1*player,3)
    for player_mill in player_mills:
        for cell in player_mill:
            new_mills = [mill for mill in state.get_board().mills() if cell in mill and mill != player_mill]
            will_win = are_same_color(MorabarabaRules,state,new_mills,-1*player,cell)
            if will_win:
                for piece in pieces:
                    inter = set([move for move in get_effective_cell_moves(state.get_board(),piece)]) & set([move for move in get_effective_cell_moves(state.get_board(),cell)])
                    if len(inter) > 0:
                        to = [i for i in inter][0]
                        return [True,piece,to]
